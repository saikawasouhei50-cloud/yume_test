...
/* --- ✨ 등급별 이미지 글로우 효과 (filter: drop-shadow) ✨ --- */
/* N 등급: 흰색/회색 은은한 빛 */
...
#room-container {
    ...
    width: 100%;
    height: 100%;
    background-color: #111827; /* bg-gray-900 색상 */
...
#room-container > div.absolute.inset-0 {
    background-size: contain !important; /* 이미지가 잘리지 않고 꽉 차게 */
    background-position: center bottom !important; /* 바닥 기준 정렬 */
...
.iso-floor {
    ...
    /* 1. 위치를 더 아래로 내립니다 (천장이 아니라 바닥 중심에 오도록) */
    top: 63%;       
    
    /* 2. 좌우 중앙 정렬 */
    left: 50%;      
    
    /* 3. 크기를 줄여서 벽 쪽으로 넣습니다 */
    width: 46%;     
    height: 90%;    
    ...
    /* 확인용 테두리 (위치 맞춘 후에는 지우거나 transparent로 변경) */
...
body.pointer-events-none {
    pointer-events: none;
        ...
        /* 탭 버튼 활성화 스타일 개선 */
.tab-button.active {
    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); /* 보라빛 그라데이션 */
...
<button onclick="switchTab('myroom')" class="pointer-events-auto 
        ...
        flex flex-col md:flex-row items-center justify-center border border-white/20 
        mb-1 md:mb-1"> <span class="transform skew-x-12 text-xs md:text-xl mb-0.5 md:mb-0">☕</span>
    <span class="transform skew-x-12 text-[8px] md:text-lg leading-none">사무소</span>
</button>
    ...
    <button id="btn-lobby-combat" onclick="switchTab('combat')" class="pointer-events-auto group 
        ...
        <span class="transform skew-x-12 inline-block text-xs md:text-4xl">독서</span>
    ...
    <div class="mt-2 flex justify-center items-center gap-4">
        ...
        <button id="auto-fill-deck" class="gacha-button bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-full shadow-lg text-sm">자동 편성</button>
            ...
            <div id="combat-in-progress-view" class="hidden">
				...
				<div class="flex justify-end items-center gap-2 mt-4">
    <button id="combat-run-button" class="gacha-button bg-yellow-600 hover:bg-yellow-700 text-black font-bold py-1 px-3 rounded-full text-sm">항복</button>
    </div>
...
<div id="visit-mode-overlay" class="hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-40 flex flex-col items-center animate-bounce">
    ...
    <button onclick="openMyRoomCharacterSelector()" class="gacha-button bg-pink-500 hover:bg-pink-600 text-white font-bold py-2 px-6 rounded-full">👥 입주 관리</button>
...
<div id="myroom-char-selector-modal" class="hidden fixed inset-0 bg-black/80 flex items-center justify-center z-50">
    <div class="bg-gray-800 p-6 rounded-lg max-w-2xl w-full h-[80vh] flex flex-col border border-pink-400">
        <div class="flex justify-between items-center mb-2">
            <h3 class="text-xl font-bold text-pink-300">마이룸 입주민 설정</h3>
            <span id="myroom-selected-count" class="text-white font-bold bg-pink-600 px-3 py-1 rounded-full text-sm">0 / 5</span>
        </div>
        <p class="text-gray-400 text-sm mb-2 text-center">사무소에 머물게 할 등장인물을 선택하세요. (최대 5명)</p>
...
document.getElementById('combat-run-button').addEventListener('click', () => {
    if (confirm("정말로 항복하고 독서를 중단하시겠습니까? (패배 처리됩니다)")) {
        fleeCombat();
    }
});
	await loadGameData();
...
async function saveGameToFirebase() {
    ...
    // ✨ [핵심 수정] 여기서 undefined를 null로 싹 변환합니다.
    ...
    if (doc.exists) {
        ...
        // ============================================================
        // ✨ [핵심 수정] 이벤트 및 레이드 데이터 로드 (안전장치 강화)
        // ============================================================
        
        // 일단 저장된 데이터를 무조건 불러옵니다. (섣불리 0으로 만들지 않음)
        ...
        // 4-1. 이벤트 ID 불일치 체크 (데이터 초기화가 아닌, 갱신이 필요한 경우)
        // CURRENT_EVENT_ID가 존재하고, 저장된 ID와 다를 때만 '이벤트 포인트' 등을 리셋 고려
        // (단, 레이드 데미지는 보스 ID를 따르므로 여기서 섣불리 지우지 않음)
        if (CURRENT_EVENT_ID && savedData.currentEventId !== CURRENT_EVENT_ID) {
            console.log(`새로운 이벤트[${CURRENT_EVENT_ID}] 감지. 점수판을 갱신합니다.`);
            // 기존 점수는 초기화하되, DB 자체를 날리진 않고 로컬 변수만 초기화
            // (다음 저장 시점에 덮어씌워짐)
            playerEventPoints = 0;
            ...
            purchasedEventItems = {};
            playerChoices = {};
            // 주의: playerRaidDamage는 여기서 초기화하지 않습니다. (보스 ID 기준이므로)
        }

        // 4-2. 레이드 횟수(Daily Count) 체크 (한국 시간 기준)
        // KST 기준 날짜가 다르면 0, 같으면 저장된 값 사용
        ...
        if (lastResetTimestamp && typeof lastResetTimestamp.toDate === 'function') {
            savedLastRaidDateStr = getTodayDateString(lastResetTimestamp.toDate());
        }
    ...
    switchTab('home');
    checkAchievements();
    checkAndSpawnRaidBoss(); // 보스 데이터 확인
...
function pullCharacter(allowedRarities = null, gachaType = 'normal') {
    ...
    const sortedInventory = [...filteredInventory].sort((a, b) => {
        ...
        // 🛡️ [안전장치] 캐릭터 데이터가 없는 유령 카드는 맨 뒤로 보냄 (에러 방지)
        ...
        // 안전장치 2: 혹시 데이터는 있는데 등급(rarity)이 비어있을 경우 대비
...
function displayMyRoom() {
    ...
    // ✨ [추가] 가구 데이터가 비어있다면 빈 배열로 초기화 (에러 방지)
    ...
    playerRoom.furniture.forEach((item, index) => {
        ...
        if (furnitureData) {
            ...
            // ✨ [수정] pointer-events-auto 추가 (가구 클릭/드래그 허용)
    ...
    // (2) SD 캐릭터 렌더링 (수정됨: 마이룸 설정 우선 -> 없으면 덱 -> 없으면 인벤토리)
    let activeCharacters = [];

    // 1순위: 마이룸에 저장된 입주민 확인
    if (playerRoom.characters && playerRoom.characters.length > 0) {
        // (A) 내 방일 때: 데이터가 ID(문자열)로 저장되어 있으므로 인벤토리에서 찾음
        if (typeof playerRoom.characters[0] === 'string') {
            activeCharacters = playerRoom.characters
                .map(id => playerInventory.find(c => c.id === id))
                .filter(Boolean);
        } 
        // (B) 친구 방일 때: visitFriendRoom에서 이미 객체로 변환해두었으므로 그대로 사용
        else {
            activeCharacters = playerRoom.characters;
        }
    }

    // 2순위: 설정된 입주민이 없거나 유효한 카드가 0장이면 -> 현재 '편찬(Deck)' 사용
    ...
    activeCharacters.forEach((card, index) => {
        ...
        // ✨ [수정] pointer-events-auto 추가 (캐릭터 클릭 허용)
        ...
        // ✅ 수정: 타일 크기(TILE_WIDTH_PERCENT)에 비례하게 설정 (1.5배 ~ 1.8배 추천)
        el.style.width = `${TILE_WIDTH_PERCENT * 1.3}%`; 
        el.style.height = 'auto'; // 높이는 비율에 맞춰 자동 조절

        el.style.zIndex = iso.zIndex + 50;

        const img = document.createElement('img');
// ▼ baseName을 name으로 변경하세요 (두 군데 모두)
...
function startStageCombat(chapterIndex, stageIndex) {
    ...
    startCombat({ chapter: chapterIndex, stage: stageIndex }, 'main_stage');
				...
				// ✅ 기존 async function startCombat 함수 전체를 이 코드로 교체하세요.
async function startCombat(indices, type) {
    ...
    if (type === 'raid') {
        monsterData = JSON.parse(JSON.stringify({
            ...
            stats: { hp: raidBossData.currentHp, atk: raidBossData.atk, def: raidBossData.def },
            imageUrl: raidBossData.imageUrl,
            isAoE: true 
        }));
        // 레이드 최대 체력 저장
        monsterData.maxHp = raidBossData.maxHp; 
        ...
        // ✨ [핵심 추가] 일반/이벤트 몬스터 최대 체력 저장
        monsterData.maxHp = monsterData.stats.hp;
    } else if (type === 'event') {
    ...
    // 1. 퇴고/개정 레벨 적용 (여기서 combatDeck 정의)
    ...
    if(combatMonsterImage) combatMonsterImage.classList.remove('shake-animation'); // 애니메이션 잔상 제거
    ...
    await runCombatLoop(combatDeck, originalBonusDeck, monsterData, indices, type); 
            ...
            // ✅ 기존 runCombatLoop 함수를 이 코드로 통째로 교체하세요.
// ✅ [수정됨] 전투 루프 함수 (항복 시 즉시 중단 기능 추가)
async function runCombatLoop(combatDeck, originalBonusDeck, monsterData, indices, type) { 
    ...
    // 몬스터나 플레이어 중 한 쪽이 전멸할 때까지 반복
    while (monsterData.stats.hp > 0 && combatDeck.some(c => c.stats.hp > 0)) {
        
        // ✨ [핵심 1] 루프 시작 시 전투 중인지 체크. 아니라면 즉시 종료.
        ...
        for (let i = 0; i < combatDeck.length; i++) {
            ...
            if (card.stats.hp > 0) {
                await delay(800);
                if (!isCombatRunning) return; // ✨ 딜레이 직후 체크 필수!
                ...
                if (skillToUse) {
                    ...
                    await delay(400);
                    if (!isCombatRunning) return; // ✨ 딜레이 직후 체크
    ...
    await delay(1000);
    if (!isCombatRunning) return; // ✨ 마지막 딜레이 후에도 체크

    monsterData.stats.def = originalMonsterDef; 
    
    // ✨ [핵심 3] 전투가 '항복'으로 인해 이미 끝났는지(false) 확인 후 정상 종료 호출
    if (isCombatRunning) {
...
function updateCombatUI(deck, originalBonusDeck, monster) {
    ...
    if (monsterMaxHp) { // Max HP를 찾았다면
        ...
        console.error("몬스터 원본 데이터를 찾을 수 없어 HP 업데이트를 건너뜁니다");
    }
}
...
function createCard(character, options = {}) {
    ...
    // 1. 글로우 클래스 설정
    const glowClass = `rarity-glow-${character.rarity}`; // ✨ [핵심] 글로우 클래스 준비
    ...
    // 1. 구글 시트에 적어둔 '커스텀 설명'이 있다면 그걸 최우선으로 보여줌
    ...
    switch(skill.type) {
        ...
        case 'buff_atk': 
            return `자신의 공격력이 10% 증가합니다.`;
        case 'debuff_atk': 
            return `적의 공격력을 10% 감소시킵니다.`;
        case 'debuff_def': 
            return `적의 방어력을 20% 감소시킵니다.`;
        default: 
            ...
            function showCardDetails(cardId) {
                ...
                character.skills.forEach((skill, index) => {
                    ...
                    // [수정] 구글 시트에 적은 설명(desc)이 있으면 그걸 쓰고, 없으면 자동 생성 함수(getSkillDescription) 사용
...
revisionPreviewDisplay.textContent = `다음 개정 시 기본 능력치 +${nextRevisionBonus}%`;
        ...
        // 버튼 클릭 이벤트 연결 (중복 방지를 위해 onclick 속성 덮어쓰기)
        reviseInkButton.onclick = reviseCardWithInkwell;
    }
				// yumecan.html - showCardDetails 함수 내부의 맨 끝부분
                ...
                // 2. 출판 조건 확인
                // (MAX_ENHANCEMENT_LEVEL 등이 정의되어 있어야 함)
                ...
                // yumecan.html 의 showCardDetails 함수 내부

// ... (이전 코드: isMaxLevel, isMaxRevision 등 확인하는 곳)
...
// ... (이후 코드: UI 표시 로직 등 그대로 유지)

                // 3. UI 표시/숨김 처리
                if (nextRarityChar) {
                    // 조건 만족 & 상위 등급 캐릭터 데이터 존재함 -> 버튼 보이기
                    ...
                    if (pubTargetText) pubTargetText.textContent = `대상: [${nextRarityChar.rarity}] ${nextRarityChar.name}`;
                    
                    // ▼▼▼ [핵심] 괄호가 빠지기 쉬운 부분입니다. 정확히 복사하세요! ▼▼▼
                    if (pubButton) {
...
function publishCard(cardId, targetCharacterData) {
    // ▼▼▼ [주의] 여기 따옴표는 숫자 1 옆에 있는 `(백틱) 이어야 합니다! ▼▼▼
    const confirmMsg = `축하합니다! 이 원고를 [${targetCharacterData.name}]으로 정식 출판하시겠습니까?\n\n(주의: 레벨과 개정 수치가 초기화됩니다.)`;
    ...
    myCard.name = targetCharacterData.name; // 이름 변경 (등급/이미지 변경됨)
    myCard.level = 0;      // +0 초기화
    myCard.revision = 0;   // 0성 초기화
    ...
    showCardDetails(cardId); // 상세창 새로고침
...
if(marketTabBtn) marketTabBtn.click();
    ...
    el.innerHTML = `
        ...
        <div class="flex flex-col gap-1">
            ${isMyTrade 
                ? `<button onclick="cancelTrade('${doc.id}')" class="bg-red-500 hover:bg-red-600 text-white text-xs px-3 py-2 rounded">회수</button>`
                // ✨ [수정] executeTrade 함수에 등급 정보(targetRarity) 추가 전달
                : `<button onclick="executeTrade('${doc.id}', '${trade.targetName}', '${trade.targetRarity || 'ALL'}')" class="bg-blue-500 hover:bg-blue-600 text-white text-xs px-3 py-2 rounded font-bold">교환</button>`
            }
        </div>
    `;
    container.appendChild(el);
        ...
        function openTradeSelectModal(tradeId, candidates, targetName, targetRarity) {
            ...
            candidates.forEach(cardInstance => {
                const charData = findCharacter(cardInstance.name);
                const revision = cardInstance.revision || 0; // 개정 수치 (없으면 0)
                const stars = '⭐'.repeat(revision); // 별 문자열 생성
                ...
                cardEl.innerHTML = `
                    ...
                    <p class="text-[10px] text-center text-gray-400">
                        Lv.${cardInstance.level} 
                        <span class="text-yellow-400">${revision > 0 ? `(+${revision}개정)` : ''}</span>
                    </p>
                `;
                list.appendChild(cardEl);
            });
...
async function visitFriendRoom(targetUid, targetNickname) {
    ...
    try {
        ...
        showMessage(`${targetNickname}님의 사무소에 방문했습니다.`, 'text-indigo-300', messageArea);
...
function exitVisitMode() {
    if (originalMyRoomData) {
        playerRoom = originalMyRoomData; // 내 데이터 복구
    ...
    // 내 방 다시 그리기
    displayMyRoom();
    showMessage("나의 탐정 사무소로 돌아왔습니다.", 'text-green-300', messageArea);
}
...
function updateMyRoomSelectorUI() {
    ...
    if (filteredInv.length === 0) {
        list.innerHTML = `<p class="col-span-full text-center text-gray-500 py-10">해당 등급의 입주민이 없습니다.</p>`;
        return;
    }
    ...
    rarityOrder.forEach(rarity => {
        ...
        if (charList && charList.length > 0) {
            ...
            if (gachaType === 'event') {
                 ...
                 if (pickupCount > 0) {
                     pickupBadge = `<span class="ml-2 text-red-400 bg-red-900/30 px-2 py-0.5 rounded-full text-sm font-medium">✨ 픽업 대상 (${pickupCount}종)</span>`;
                 }
            }
